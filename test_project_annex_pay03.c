...
/* ANNEX PAY - A Banking System of BUBT in C language.
   Features:
...
void loading(const char *msg)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        printf(".");
        fflush(stdout);
        /* Longer delay - approximately 0.3 seconds per dot */
...
/* Enhanced Bill payment with university fees and other fees options */
void billPayment(int accNo)
{
    ...
    if (choice == 4) /* University Fees */
    {
        char studentId[20];
        printf("Enter Student ID: ");
        scanf("%s", studentId);
        printf("Enter university fees amount: ");
        scanf("%f", &amt);
        snprintf(note, sizeof(note), "University Fees for Student ID: %s", studentId);
    }
    else if (choice == 5) /* Other Fees */
    {
        char feeName[50];
        printf("Enter fee name: ");
        getchar(); /* Consume newline */
        fgets(feeName, sizeof(feeName), stdin);
        feeName[strcspn(feeName, "\n")] = '\0';
        printf("Enter fee amount: ");
        scanf("%f", &amt);
        snprintf(note, sizeof(note), "Other Fees: %s", feeName);
    }
    else /* Regular bills */
    {
        printf("Enter bill amount: ");
        scanf("%f", &amt);
        const char *billTypes[] = {"", "Electricity", "Water", "Internet"};
        if (choice >= 1 && choice <= 3)
        {
            snprintf(note, sizeof(note), "Bill type: %s", billTypes[choice]);
        }
    }
